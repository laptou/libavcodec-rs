...
impl<D> IoContext<D> {
    ...
    pub fn new(data: D, params: IoContextParams<D>) -> crate::Result<Self> {
        ...
        ctx.initialize(buffer_size)?;

        Ok(ctx)
    }

    fn initialize(&mut self, buffer_size: usize) -> crate::Result<()> {
        unsafe {
            ...
            self.inner = NonNull::new(context).ok_or(crate::Error::Alloc)?;
        }
